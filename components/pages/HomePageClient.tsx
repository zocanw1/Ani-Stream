...
import {
  CalendarDays, ChevronLeft, ChevronRight, CirclePause, CirclePlay,
  Info, Play, Star, TrendingUp, Sparkles, Film, Clock,
} from "lucide-react";
...
type SamehadakuAnime = {
  ...
  status: string; animeId: string; href: string;
  samehadakuUrl: string; genreList?: Genre[];
  releasedOn?: string; episodes?: string;
};
type ScheduleItem = {
  title: string; poster: string; type: string;
  score: string; estimation: string; genres: string;
  animeId: string; href: string;
};
...
function SectionHeading({ icon: Icon, title, description, href, premium = false }: {
  icon: typeof TrendingUp; title: string; description?: string; href?: string; premium?: boolean;
}) {
  return (
    <div className="catalog-heading">
      <div>
        <span className="catalog-heading__icon">
          {premium ? (
            <span className="relative flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-[var(--primary)]/20 to-[var(--secondary)]/20 border border-[var(--border-glow)]">
        ...
        <div>
          <h2 className={premium ? "text-gradient-anime" : ""}>{title}</h2>
          {description && <p>{description}</p>}
        </div>
      </div>
      {href && (
        <Link href={href} prefetch={false} className="group flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.15em] transition-colors" style={{color: "var(--text-secondary)"}}>
          Lihat Semua
...
export default function HomePageClient({ initialData, initialError = null }: {
  ...
  useEffect(() => {
    ...
    async function fetchInitialData() {
      try {
        ...
        if (![homeRes, scheduleRes, popularRes, ongoingRes].every((response) => response.ok))
          throw new Error("Gagal mengambil data dari server");
        const [home, schedule, popular, ongoing] = await Promise.all([
          homeRes.json(), scheduleRes.json(), popularRes.json(), ongoingRes.json(),
        ]);
        setData({
          homeData: home.data,
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(
      () => setCurrentSlide((slide) => (slide + 1) % data.popularData.length),
      6500,
    );
  ...
  const activeDayList = useMemo(
    () => data?.scheduleData.find((entry) => entry.day === activeDay)?.animeList ?? [],
      ...
      <section className="cinematic-hero" aria-label="Anime pilihan">
        {heroAnime ? (
          <>
            ...
            <div className="cinematic-hero__inner">
              <div className="cinematic-hero__copy">
                <div className="hero-eyebrow">
                  <span className="flex items-center gap-1.5">
                    <Sparkles size={12} /> #{currentSlide + 1} Populer
                  </span>
                  {scoreValue(heroAnime.score) && (
                    <span style={{color: "var(--gold)"}}>
                      <Star size={12} fill="currentColor" /> {scoreValue(heroAnime.score)}
                    </span>
                  )}
                ...
                <div className="hero-genres">
                  {heroAnime.genreList?.slice(0, 4).map((genre) => (
                    <span key={genre.genreId}>{genre.title}</span>
                  ))}
                </div>
                <p className="text-[var(--text-secondary)] text-sm leading-relaxed">
                  Ikuti kisah {heroAnime.title} dan temukan episode terbaru dengan subtitle Indonesia.
                </p>
                <div className="hero-actions">
                  <Link
                    href={`/anime/${cleanSlug(heroAnime.href)}`}
                    prefetch={false}
                  ...
                  <Link
                    href={`/anime/${cleanSlug(heroAnime.href)}`}
                    prefetch={false}
                    className="btn-secondary inline-flex items-center gap-3 px-8 py-3.5 text-xs font-black uppercase tracking-[0.15em]"
                  >
                    <Info size={18} /> Detail Anime
                  </Link>
                </div>
              </div>
              <div className="cinematic-hero__poster">
                <Image
                  ...
                  alt={heroAnime.title}
                  fill
                  sizes="(max-width: 1024px) 0px, 270px"
                  priority
                  className="object-cover"
                />
              </div>
            ...
            <div className="hero-carousel-controls">
              <button
                ...
                aria-label={carouselPaused ? "Putar carousel" : "Jeda carousel"}
              >
              ...
              {data.popularData.map((anime, index) => (
                <button
                  ...
                  aria-label={`Tampilkan ${anime.title}`}
          ...
          <div className="cinematic-hero__fallback">
            <span className="text-[var(--primary)] text-xs font-black uppercase tracking-[0.2em]">
              ✦ AniStream
            </span>
            <h1 className="text-gradient-anime text-5xl sm:text-6xl lg:text-7xl font-black leading-[0.95] mt-4">
              Anime favoritmu,<br />lebih cepat ditemukan.
            </h1>
            <p className="text-[var(--text-secondary)] text-sm leading-relaxed max-w-xl mt-4">
              Jelajahi jadwal rilis, episode terbaru, film, dan daftar tontonan dari satu tempat.
            </p>
            <div className="hero-actions">
              <a href="#jadwal-rilis" className="btn-glow inline-flex items-center gap-3 px-8 py-3.5 text-xs font-black uppercase tracking-[0.15em]">
                <CalendarDays size={18} /> Lihat Jadwal
              </a>
            </div>
          </div>
        )}
      ...
      <div className="cinematic-content">
        ...
        {error && (
          <div className="state-panel state-panel--error" role="alert">
            <div>
              <strong className="text-sm font-black">Data anime belum dapat dimuat.</strong>
        ...
        {/* ═══════ SCHEDULE PREMIUM ═══════ */}
        <section id="jadwal-rilis" className="schedule-panel animate-slide-up" aria-labelledby="schedule-title">
          <SectionHeading
            icon={CalendarDays}
            title="Jadwal Rilis Minggu Ini"
            description="Pilih hari untuk melihat anime yang tayang"
            premium
          />
          ...
          <div className="schedule-strip">
            {activeDayList.map((anime) => (
              ...
              >
                <span className="schedule-item__poster">
                  <Image src={anime.poster} alt="" fill sizes="56px" className="object-cover" />
                </span>
                <span>
                  <strong>{anime.title}</strong>
                  <small>{anime.estimation || anime.type}</small>
                </span>
                <ChevronRight size={16} />
              </Link>
            ))}
            {!activeDayList.length && !loading && (
              <p className="schedule-empty">Tidak ada jadwal pada hari ini.</p>
            )}
          </div>
        </section>

        {/* ═══════ TOP 10 PREMIUM ═══════ */}
        <section className="catalog-section animate-slide-up" aria-labelledby="top-title">
          <SectionHeading
            icon={TrendingUp}
            title="Top 10 Minggu Ini"
            description="Judul yang sedang banyak ditonton"
            href="/popular"
            premium
          />
        ...
        <div className="home-catalog-grid">
          <section className="catalog-panel animate-slide-up">
            <SectionHeading icon={Play} title="Episode Baru" description="Update terkini" />
          ...
          <section className="catalog-panel animate-slide-up">
            <SectionHeading icon={Sparkles} title="Sedang Tayang" description="Ongoing series" />
            <div className="catalog-panel__grid">
          ...
          <section className="catalog-panel animate-slide-up">
            <SectionHeading icon={Film} title="Film Anime" description="Movie collection" />
            <div className="catalog-panel__grid">
